...
To further illustrate how the individual factor levels differ from each other the detailed results of Models \eqref{eqn:demographic_response} and \eqref{eqn:demographic_time} are shown in Table \ref{tbl:model_result_demographics}. For parameter estimation the first factor levels shown in the Table are are used as the base line. The effect of all the demographic factors on log time taken to evaluate a lineup appears to be significant. But for probability of correct responses we observe that not all the factor levels are significant. 

\begin{table}[htbp]
\centering
\caption{Parameter estimates of Models \eqref{eqn:demographic_time} and \eqref{eqn:demographic_response} fitted for avarage log time taken and probability of correct lineup evaluations respectively. For time taken all the demographic factors appear to be significant. For probability of correct response age group 36-40, rest of the world and graduate degree appears to be significantly different. Gender does not show any difference in performance.}
\scalebox{0.9}{
...
Notice that lineup specific variance estimate is 5.259 with a standard error of 2.293 which is much higher than the other significant parameter estimates in Model \eqref{eqn:demographic_response}. This indicates that the major and most important factor affecting the probability of correct response is the lineup difficulty. This is also evident from the large variability in the proportion correct in Figure~\ref{fig:demographic_effect}. Because of this huge impact of lineup specific variance, the actual effect of other factor variables on the probability of correct response is in fact very small even though they appear to be significant. We illustrate this with the following example of the significant effect of graduate degree.

Effect of graduate degree is estimated as 0.182 which appears to be highly significant. This causes an increase of probability of correct response for graduate degree over high school by 
$$ \frac {\exp(-0.683+0.182)}{1+\exp(-0.683+0.182)}-  \frac {\exp(-0.683)}{1+\exp(-0.683)} = 0.377- 0.336 =0.042.$$  If we move away one standard deviation of lineup variation, in other words if we add 2.293 to teach of the exponent term in above equation, this increase reduces to 0.024. For two standard deviations it further reduces to 0.003 which is practically very small. The similar calculation shows the same negligible impact of age level 36-40 and country on the probability of correct response. Thus even though some of the factor levels are significant, their practical impact is negligible because of high variability of lineup difficulties.
...
Observer demographics seem to have significant influence on performance in time taken to evaluate a lineup. Some variations among the factors are observed in terms of probability of correct evaluation. Age group 36-40, Countries other than India and United states, People who have a graduate degree appear to have significantly more probability to evaluate a lineup correctly. Gender does not have any effect on performance.

Individual learning trend is observed in both time taken and observer performance. Some individuals improved the performances while others showed decrease in their performances over attempts. But the overall performance of human observers does not increase through successive attempts while evaluating multiple lineups. The skill in evaluating the lineup in shorter time gets improved over successive evaluations. The earlier evaluations take significantly longer than the later evaluations. This result is very important as it suggests that the power estimated for visual inference using human subject experiment is robust and may not change if those participants are allowed to give feedback again. This investigation suggest that the skilled person may only do it faster.
