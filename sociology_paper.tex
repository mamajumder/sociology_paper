...
There have been \dc{ten} experiments conducted using Amazon's Mechanical Turk~\cite{turk} that are being used to evaluate the the effect of the observer. Table \ref{tbl:visual_stat} summarizes these. Experiments XXX are used to evaluate the effects of demographic factors whethere there is a learning trend. The design of experiment XXX enabled effect of position in the lineup, and the sample of nulls, to be evaluated. Section \ref{sec:factor_performance} describes the factors that may affect the performance of the observer. Section \ref{sec:exp_design} describes the methods used to assess the effects, and Section \ref{sec:result_socio} describes the results.
...
2 &  Scatter plot & \begin{minipage}[t]{3cm}   \scalebox{0.45}{\includegraphics{stat_beta_k.pdf}}\end{minipage} & Of the scatter plots below which one shows data that has steepest slope? \\ 
  ...
  3 & Contaminated plot &  \\ 
  4 & Polar vs Cartesian &  \\ 
  5 & Hist vs density &  \\ 
  6 & Violine vs boxplot &  \\ 
  7 & Group separation &  \\ 
  8 & Sine Illusion &  \\ 
  9 & Gene expression &  \\ 
  10 & Test normality &  \\ 
