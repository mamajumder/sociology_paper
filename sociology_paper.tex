...
%
\hh{XXX -- the way that the random effects are listed is a bit misleading -- the first value is the {\bf variance} component, not the estimate of the standard deviation, while  the second value is its square root value, not the standard error of the estimate. I don't believe that lmer provides a standard error of the variance component estimates.}}
\scalebox{0.86}{
...
\begin{table}[htbp]
\centering
\caption{Parameter estimates of model \eqref{eqn:trend_time} fitted for log time taken to evaluate a lineup. Both fixed effect parameters of Attempt ($\alpha_1$ and $\alpha$) are highly significant for all three experiments 5, 6 and 7. \hh{XXX same problem with the random effects as in tables 4 and 5.}}
\scalebox{0.81}{
\begin{tabular}{rrrrccrrrccrrrc}
  \hline
& \multicolumn{4}{c} {Experiment~5} & &\multicolumn{4}{c} {Experiment~6} && \multicolumn{4}{c} {Experiment~7}\\
