...
\item{\bf Question Design:} A primary purpose of visual inference is to preclude the necessity to pre-specify discoverable features in data that is required by classical statistical inference. Visual inference supports discovery of srtucture, enabling the the unexpected to surprise us.  This is tightly connected to the task that the human observer is asked to perform. For most purposes the oberver is asked a very general question, such as to pick the plot that is most different from the others, and explain the reasons why they see it as different. By being general, all possible discoverable structures are included in the significance calculations. (\citet{buja:2009} has a very complete explanation.)
